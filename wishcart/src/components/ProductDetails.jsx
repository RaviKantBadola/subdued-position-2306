...
const fetchData = async () => {
    ...
    try {
        const res = await fetch(`https://different-bell-bottoms-fox.cyclic.app/wishcart?item_id=${item_id}`)
    ...
    <HStack p="10px" display={{ base: 'block', md: 'flex', lg: 'flex' }}>
                ...
                <Box display={"flex"} p="15px">
                    <Box w="15%">
                        <Box display={{ base: "none", md: 'block', lg: "block" }} p='7px' border={"2px solid #2974F1"} m="auto" >
                            <Img w={{ base: "0px", md: "30px", lg: "45px" }} m='auto' src={item.image||item.imageUrl[0]} alt="smallImg" />
                        </Box>
                    </Box>
                    <Box w="79%" p='10px' minH={'400px'} display='flex' justifyContent={'center'} alignItems={'center'} >
                       
                        <Img  maxH={{base:"500px",md:"300px", lg:"100%"}} mw='450px' m='auto' src={item.image||item.imageUrl[0]} />
                    </Box>
        ...
        {viewData.map((item, index) => (
            <Box key={index} w={{ base: '100%', md: '60%', lg: '60%' }} pt="-50px" h="100vh" overflow={{ base: "none", md: "auto", lg: "auto" }} pl="2%" className='example'>
                ...
                <Box> <Text textAlign={"left"} color={"silver"} > {item.brand} </Text></Box>
                <Text textAlign={"left"} fontWeight={"500"}> {item.description}</Text>
                <Text textAlign={"left"} fontSize={"12px"} color="green"> Special price</Text>
                ...
                <Box pt="20px" fontSize={"12px"}>  <Text textAlign={"left"}>  Usually delivered in 8 days</Text>
                    <Text textAlign={"left"}>  Enter pincode for exact delivery dates/charges</Text>
                </Box>
                <Text color='#2974F1' pt={"10px"} fontSize={{ base: '8px', md: '10px', lg: '15px' }} textAlign={"left"}> View Details</Text>
